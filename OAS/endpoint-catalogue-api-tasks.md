...
- [x] **[#40] connection-type/payload-type names in examples/prose** — reviewer flagged stale PascalCase in examples. Fixed the two URL-encoded example strings (`ConnectionType=`→`connection-type=`, `PayloadType=`→`payload-type=`) and the `$template` operation summary prose. Component keys/`$ref`s (`ConnectionType_QParam`) left as-is (internal, off-wire). ✅ Done.
- [x] **[#41] POST /List `201` → `200` (BaRS-aligned)** — reviewer feedback #1: decided to keep BaRS-aligned creates (`200`, no `Location`), so brought `POST /List` into line — changed its `201`/`201-AtomicList` response to `200`/`200-AtomicList`. All POST and PUT operations now return `200`. ✅ Done. Note: the `201-Atomic*` response components (incl. `201-AtomicList`) are now orphaned — candidate for cleanup.
- [x] **[#42] GET /HealthcareService "at least one of" guidance** — reviewer feedback #4: added guidance that at least one of `identifier` or `organization.identifier` must be supplied (both individually optional, combinable as AND; neither → `400 Bad Request`; use `GET /HealthcareService/{id}` for single read). Updated the operation description (with examples) and both param descriptions. Params stay `required: false` (OpenAPI can't express "at least one of" — documented + server-side enforced). Also corrected stale `connectionType`/`payloadType` → `connection-type`/`payload-type` in that description. ✅ Done.
- [x] **[#43] Endpoint.header example changed** — reviewer feedback #3: the `Endpoint.header` example `Content-Type: application/fhir+json` was being confused with the API's own Accept/Content-Type request headers. Changed all 11 occurrences (schemas + examples) to `Authorization: Bearer <access-token>`, which clearly reads as a connection header for the *target endpoint's* address (the true meaning of FHIR `Endpoint.header`). ✅ Done.
- [x] **`_id` search additions** — **Decision: both removed.** `GET /HealthcareService?_id` was removed (redundant with `GET /HealthcareService/{id}`), and `_has:HealthcareService:endpoint:_id` on `GET /Endpoint` was removed (use `_has:...:identifier`). OAS confirmed clean — no `_id` search remnants remain. ✅ See the `#35/#36` reconciliation item below for detail.
