# EPC-INV010 - Audit & Observability
...
2. WHEN a `POST /HealthcareService` operation automatically creates an associated `List` resource, THE Audit_Service SHALL create two Audit_Records: one for the `HealthcareService` creation and one for the `List` creation, each with `changeType: created`.

3. WHEN a `PUT` or `PATCH` on a `HealthcareService` automatically synchronises the associated `List`, THE Audit_Service SHALL create an Audit_Record for the `List` update with `changeType: updated`, in addition to the Audit_Record for the `HealthcareService` change.
...
| actorId | string | User identifier from the Bearer_Token |
...
2. WHEN the changed resource is an `Endpoint` or `Template`, THE Audit_Service SHALL also record the following additional fields on the Audit_Record where they are present on the resource at the time of the change:

| Field | Description |
|-------|-------------|
| endpointStatus | The status value of the Endpoint or Template after the change |
| endpointUrl | The address field of the Endpoint or Template |
| connectionType | The connectionType code |
| payloadType | The payloadType code(s) |
| productId | The Product_Identifier, if present |

3. WHEN the changed resource is an `Endpoint`, THE Audit_Service SHALL also record the `healthcareServiceId` of the HealthcareService that references the Endpoint at the time of the change.

4. WHEN the changed resource is a `List`, THE Audit_Service SHALL also record the `healthcareServiceId` of the HealthcareService referenced by `List.subject`.

5. THE Audit_Service SHALL record the state of the resource **after** the change for `created` and `updated` events, and the state **before** the change for `deleted` events.
