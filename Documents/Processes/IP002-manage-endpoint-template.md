...
> ⚠️ **Requirements conflict — Template status mutability:**
>
> The Endpoint Catalogue Service Requirements V1.8 (Section 4.2, Additional Implementation
> Details) states: *"The status of an endpoint template is fixed i.e. cannot be modified."*
>
> EPCFUNC-15 lists only `product ID`, `name`, and `address (url)` as updatable attributes
> on a Template — **status is explicitly excluded**.
>
> EPCFUNC-21 states: *"The solution MUST prevent the deletion of any Organisation,
...
> - If a supplier wants to take all their endpoints offline (e.g., during a major
>   outage), the requirements imply they must update each child Endpoint's status
>   individually — there is no "suspend the Template and cascade to children" mechanism
>
> **This is a design gap.** The operational need exists (supplier outage → turn off all
> endpoints quickly), but the requirements don't provide for it. Options:
>
> 1. **Challenge the requirement** — argue that Template status mutability is
>    operationally necessary and request a change to the requirements
> 2. **Implement a bulk endpoint update** — provide a mechanism to suspend all child
>    Endpoints of a Template in one operation, without changing the Template itself
> 3. **Use visibility rules** — if the Template status is used in the EPC's visibility
>    rules (Template `status` must be `active` for child Endpoints to be visible), then
>    changing Template status would effectively hide all children without modifying them.
>    This is what the current document describes but it contradicts the requirements.
>
> **Decision needed:** Confirm with the programme whether Template status should be
> mutable. If yes, update the requirements. If no, remove the soft/hard delete
> functionality from this document and replace with a bulk child Endpoint suspension
> mechanism.
