...
| `Status` present                   | Non-empty, must be a valid code from the FHIR [endpoint-status](http://hl7.org/fhir/R4/valueset-endpoint-status.html) code system, but cannnot be `entered-in-error` as that would be a soft delete, which is covered below | Do not proceed if invalid | `FAILED` — "Invalid Status: {value}. Must be a valid FHIR endpoint-status code" |     |
...
| [Daily Switch Process](./daily-switch-process.md)                                    | Pharmacy supplier switch process (CSV-driven batch switching) |
